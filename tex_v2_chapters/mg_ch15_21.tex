...
self-resonance under extension (Axiom~A3 of \emph{Idea Diffusion Theory}), and
...
\label{thm:poetic-weight-ratchet}
...
\emph{ritual repetition creates sacredness}.  Roy Rappaport
...
The strategy space $\mathcal{P}(\IS)$ is
...
\begin{proof}
Adapt the Myerson--Satterthwaite impossibility theorem.
...
Myerson-type auction where each idea~$a_i$
...
selfish meaning-making.  For congestion games it is bounded in the
classical Roughgarden--Tardos setting; for meaning games, the bound likely
...
that invoke ``integrated information'' (IIT, as in Tononi's work)?
